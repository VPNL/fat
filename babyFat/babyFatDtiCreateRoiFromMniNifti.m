function [RoiFileName, invDef, roiMask]=babyFatDtiCreateRoiFromMniNifti(dt6File, ROI_img_file, invDef, saveFlag, roiNum, showFigs, tdir, template)
...
if saveFlag
    ...
    rDir = fullfile(fileparts(dt6File),'babyAFQROIs');
    if ~exist(rDir,'dir'); mkdir(rDir); end
    RoiFileName = fullfile(fileparts(dt6File), 'babyAFQROIs', [prefix(ROI_img_file, 'short') '.mat']);

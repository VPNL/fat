function bigImg = babyFatMontage(fatDir, sessid, runName, imgName,nC,outdir)
...
for r = 1
    ...
    for s = 1:length(sessid)
        runDir = fullfile(fatDir,sessid{s},runName{r},'dti94trilin');
...
%     txtImg = export_fig('-native');
    ...
    % write image
    if ~isempty(img)
    imwrite(img,fullfile(outdir,sprintf('montage_%s',imgName)),'Resolution', 600);
    end
end

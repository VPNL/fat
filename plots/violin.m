...
function[h,L,MX,MED,bw]=violin(Y,varargin)
 ...
 for i=1:size(Y,2)
    ...
    F(:,i)=f;
    U(:,i)=u;
    % Instead of plotting median, we'll plot standard error of the mean
    % We'll calculate the upper (u) and (l) of STE
    MEDu(:,i)= nanmean(Y{i}) + nanstd(Y{i})/(sqrt(length(Y{i}))); 
    MEDl(:,i)= nanmean(Y{i}) - nanstd(Y{i})/(sqrt(length(Y{i}))); 
 ...
 for i=i:size(Y,2)
   ...
   if setX == 0
    if plotmean == 1
     p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i)) ],[MX(:,i) MX(:,i)],mc,'LineWidth',3);
    end
    if plotmedian == 1
     p(2)=plot([interp1(U(:,i),F(:,i)+i,MEDu(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MEDu(:,i)) ],[MEDu(:,i) MEDu(:,i)],'--k','LineWidth',1);
     p(2)=plot([interp1(U(:,i),F(:,i)+i,MEDl(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MEDl(:,i)) ],[MEDl(:,i) MEDl(:,i)],'--k','LineWidth',1);
    end
   elseif setX == 1
    if plotmean == 1
 ...
 if plotlegend==1 & plotmean==1 | plotlegend==1 & plotmedian==1
  ...
  if plotmean==1 & plotmedian==1
   L=legend([p(1) p(2)],'Mean','Std. Error');

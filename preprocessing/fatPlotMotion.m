function [fh, figurename] = fatPlotMotion(eddyMotionFile,visibility)
...
plot(motion(:,1:3)); 
title('Translation'); 
xlabel('Diffusion excitation'); 
ylabel('translation (mm)'); 
legend({'x','y','z'});

subplot(2,1,2); 
plot(motion(:,4:6)/(2*pi)*360); 
title('Rotation');
xlabel('Diffusion excitation'); 
ylabel('rotation (deg)'); 
...
d = vertcat(0, diff(sqrt(motion(:,1).^2+motion(:,2).^2+motion(:,3).^2)));

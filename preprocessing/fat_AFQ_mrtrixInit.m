function files = fat_AFQ_mrtrixInit(dt6, ...
    mrtrix_folder, ...
    mrtrixVersion, ...
    multishell, ...
    tool, ...
...
if (~computed.('dt'))
    ...
    cmd_str = ['mrconvert ' files.dt ' ' dt_info.files.tensors ' -grad ' files.b ' -force -quiet'];
    [status,results] = AFQ_mrtrix_cmd(cmd_str, 0, 1,mrtrixVersion);
end
...
if (~computed.('fa'))
    ...
    cmd_str = ['mrconvert ' files.fa ' ' dt_info.files.fa ' -grad ' files.b ' -force -quiet'];
    [status,results] = AFQ_mrtrix_cmd(cmd_str, 0, 1,mrtrixVersion);
end
...
%     end
    ...
    % Compute the CSD estimates:
    if (~computed.('wmCsd'))   && (mrtrixVersion > 2)
        cmd_str = ['dwi2fod msmt_csd ' ...
            files.dwi ' ' ...
            files.wmResponse  ' '  files.wmCsd ' '  ...
